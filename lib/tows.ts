export type Kind = 'gate' | 'bse-in' | 'bse-out' | 'long' | 'manual' | 'none' | 'incomplete';
...
export function longMoves(t:Turn,holding:string):Move[]{
 const stand=towLocation(holding);if(!stand||stand===towLocation(t.from))throw Error('Choose a holding stand different from the arrival gate.');
 ...
 return [{...base,id:t.id+'-out'},{...base,id:t.id+'-return',arrFlight:'',depFlight:t.depFlight,from:stand,to:t.to,pickup:clock(roundPickup(t.departure===null?null:t.departure-60*60000)),depTime:clock(t.departure),reason:'Long stay · return to departure gate'}];
}
export function moveErrors(m:Move,date:string):string[]{
 const errors:string[]=[];
 if(!m.fin.trim())errors.push('Aircraft number is required.');if(!m.from.trim()||!m.to.trim())errors.push('Both tow locations are required.');else if((m.kind==='long'||m.kind==='manual'?towLocation(m.from)===towLocation(m.to):gate(m.from)===gate(m.to)))errors.push('Tow locations must be different.');
...
export function planIssues(moves:Move[]):string[]{
 ...
 if(out?.included&&back?.included){if(out.pickup>=back.pickup)issues.push(`FIN ${out.fin}: holding tow must be before the return tow.`);if(towLocation(out.to)!==towLocation(back.from)||towLocation(out.from)!==towLocation(back.to)||out.fin!==back.fin)issues.push(`FIN ${out.fin}: the holding and return routes must match.`);}}
 return issues;
}
