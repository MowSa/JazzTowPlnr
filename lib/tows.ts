export type Kind = 'same-area' | 'gate' | 'bse-in' | 'bse-out' | 'long' | 'manual' | 'none' | 'incomplete';
...
export function towLocation(v:string){let s=(v.split('/').pop()||'').trim().toUpperCase();if(['','-','N/A'].includes(s))return '';s=s.replace(/^[AC](?=\d)/,'');return /^\d+$/.test(s)?String(Number(s)).padStart(2,'0'):s;}
export function gateArea(value:string):'US'|'Domestic'|null {
 const normalized=towLocation(value);if(!/^\d+$/.test(normalized))return null;const n=Number(normalized);
 return n>=1&&n<=49?'Domestic':n>=56&&n<=58||n>=73&&n<=89?'US':null;
}
export function clock(t:number|null){if(t===null)return '';return new Date(t).toISOString().slice(11,16);}
/** Round a scheduled pickup to the closest five-minute interval (e.g. 18:03 → 18:05). */
export function roundPickup(t:number|null){return t===null?null:Math.round(t/(5*60000))*5*60000;}
...
export function analyze(text:string,dateOverride?:string):Report {
 ...
 const arrival=stamp(r[2],date),departure=stamp(r[11],date);const departureActual=departure!==null&&/\sA\s*$/.test(r[11]);const aToday=arrival!==null&&arrival>=start&&arrival<end,dToday=departure!==null&&departure>=start&&departure<end;
 ...
 const fin=r[6]==='-'?'':r[6];
 if(departureActual){kind='none';reason='Departure time is actual. A tow move is no longer required.';}
 else if(!fin||arrival===null||departure===null||duration!==null&&duration<0){kind='incomplete';reason='Incomplete or inconsistent aircraft/timing data. Confirm this turn manually.';}
 ...
 else if(from!==to){const area=gateArea(from);kind=area&&area===gateArea(to)?'same-area':'gate';reason=kind==='same-area'?`${area} to ${area}: confirm a direct tow or choose an intermediate holding stand.`:`Arrival gate ${from} differs from departure gate ${to}.`;}
...
export function makeMoves(report:Report):Move[]{
 ...
 const outbound=t.kind==='bse-out',inbound=t.kind==='bse-in';const pickup=roundPickup(outbound&&t.departure!==null?t.departure-60*60000:t.arrival);const outside=pickup!==null&&(pickup<start||pickup>=end);
 ...
 return {id:t.id,turnId:t.id,fin:t.fin,arrFlight:outbound?'':t.arrFlight,depFlight:inbound?'':t.depFlight,from:outbound?'BSE':t.from,to:inbound?'BSE':t.to,pickup:outside?'':clock(pickup),release:'',gateOpen:'',actualPickup:'',actualDrop:'',depTime:inbound?'':clock(t.departure),tower:'',kind:t.kind,included:t.kind!=='incomplete',reviewed:false,reason:t.reason,earliest:outbound?null:pickup,latest:inbound?null:t.departure,warnings};
 });
}
export function longMoves(t:Turn,holding:string):Move[]{
 const stand=towLocation(holding);if(!stand||stand===towLocation(t.from)||stand===towLocation(t.to))throw Error('Choose a holding stand different from both flight gates.');
 const pickup=roundPickup(t.arrival);const base:Move={id:t.id,turnId:t.id,fin:t.fin,arrFlight:t.arrFlight,depFlight:'',from:t.from,to:stand,pickup:clock(pickup),release:'',gateOpen:'',actualPickup:'',actualDrop:'',depTime:'',tower:'',kind:t.kind==='same-area'?'same-area':'long',included:true,reviewed:false,reason:t.kind==='same-area'?'Same-area transfer · tow to holding stand':'Long stay · tow to holding stand',earliest:pickup,latest:t.departure,warnings:[...t.warnings]};
 return [{...base,id:t.id+'-out'},{...base,id:t.id+'-return',arrFlight:'',depFlight:t.depFlight,from:stand,to:t.to,pickup:clock(roundPickup(t.departure===null?null:t.departure-60*60000)),depTime:clock(t.departure),reason:t.kind==='same-area'?'Same-area transfer · tow to departure gate':'Long stay · return to departure gate'}];
}
export function moveErrors(m:Move,date:string):string[]{
 const errors:string[]=[];
 if(!m.fin.trim())errors.push('Aircraft number is required.');if(!m.from.trim()||!m.to.trim())errors.push('Both tow locations are required.');else if((m.kind==='long'||m.kind==='same-area'||m.kind==='manual'?towLocation(m.from)===towLocation(m.to):gate(m.from)===gate(m.to)))errors.push('Tow locations must be different.');
...
export function exportCSV(report:Report,moves:Move[],draft:boolean){const safe=(v:string)=>'"'+(/^[=+\-@\t\r]/.test(v)?"'"+v:v).replace(/"/g,'""')+'"';const rows=[['TOW SHEET',report.date,report.station,draft?'DRAFT — REVIEW REQUIRED':'REVIEWED'],sheetHeaders,...moves.map(sheetValues)];return '\uFEFF'+rows.map(r=>r.map(safe).join(',')).join('\r\n');}
export function planIssues(moves:Move[]):string[]{
 const issues:string[]=[];const holding=moves.filter(m=>m.kind==='long'||m.kind==='same-area');
 ...
 if(out?.included&&back?.included){if(out.pickup>=back.pickup)issues.push(`FIN ${out.fin}: holding tow must be before the return tow.`);if(towLocation(out.to)!==towLocation(back.from)||out.kind==='long'&&towLocation(out.from)!==towLocation(back.to)||out.fin!==back.fin)issues.push(`FIN ${out.fin}: the holding and return routes must match.`);}}
 return issues;
}
